...
describe('Query string validations', (done) => {
    it('Empty query string', () => {
        request(app)
        .get('/ledger')
        .expect(400, {  msg: 'start_date not present,end_date not present,frequency not present,weekly_rent not present'}, done);
    });
    it('query string with start_date', (done) => {
        request(app)
        .get('/ledger?start_date=2020-01-02')
        .expect(400, { msg: 'end_date not present,frequency not present,weekly_rent not present' }, done);
    });
    it('query string with start_date, end_date', (done) => {
        request(app)
        .get('/ledger?start_date=2020-01-02&end_date=2021-05-18')
        .expect(400, { msg: 'frequency not present,weekly_rent not present' }, done);
    });
    it('query string with start_date, end_date, frequency', (done) => {
        ...
        .expect(400, { msg: 'weekly_rent not present' }, done);
    });
    ...
    it('query string with start_date, end_date, frequency, weekly_rent, timezone', (done) => {
        request(app)
        .get('/ledger?start_date=2020-01-02&end_date=2020-03-18&frequency=WEEKLY&weekly_rent=324')
        .expect(200, [ 
